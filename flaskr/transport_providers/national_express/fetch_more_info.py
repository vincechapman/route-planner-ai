...
from flaskr.transport_providers.national_express import q
from flaskr.transport_providers.national_express.update_nx_stations import get_sqlite_db
...
def fetch_more_details(stop_id):
    ...
    if not len(q):
        print('\nQueue is empty!\n')
        from flaskr.transport_providers.national_express.upload_to_postgres import add_to_postgresql
        db.close()
        add_to_postgresql()
    else:
